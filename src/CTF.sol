...
interface Ilevel {
    ...
    ) external payable returns (uint8, uint256);
}
...
contract CTF {
    ...
    function submitSolution(
        ...
        require(
            solutions[solutionAddress] == NULL ||
                solutions[solutionAddress] == msg.sender,
            "msg.sender not owner of solution"
        );
    ...
    function addLevel(
        ...
        require(
            levels[levelNumber] == NULL,
            "Already exsists, delete or use update"
        );
    ...
    function getScore(
        address addr,
        uint8 level
    ) public view returns (address, string memory, uint8, uint8, uint256) {
        return (
            addr,
            userNames[addr],
            level,
